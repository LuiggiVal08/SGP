...
import { registerProfessorSchema, type RegisterProfessorFormData } from '@/features/admin/schemas/register-professor.schema';
...
export default function RegisterProfessorPage() {
  usePageTitle('Admin - Registrar Docente');
  ...
  } = useForm<RegisterProfessorFormData>({
  ...
  const mutation = useMutation({
    mutationFn: (data: RegisterProfessorFormData) => catalogService.registerProfessor(data),
    onSuccess: () => {
        ...
        <Card.Content className="p-6 space-y-4">
          <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                <FieldLabel label="Nombre" help="Nombre(s) del docente" htmlFor="firstName" className="text-sm" />
              ...
              <div className="flex flex-col gap-1">
                <FieldLabel label="Apellido" help="Apellido(s) del docente" htmlFor="lastName" className="text-sm" />
            ...
            <div className="flex flex-col gap-1">
              <FieldLabel label="Email" help="Correo electrónico del docente" htmlFor="email" className="text-sm" />
            ...
            <div className="flex flex-col gap-1">
              <FieldLabel label="Especialización" help="Área de especialización" htmlFor="specialization" className="text-sm" />
              <Input id="specialization" {...register('specialization')} placeholder="Ej: Matemáticas" />
            ...
            {mutation.isSuccess && (
              <p className="text-success text-sm text-center">Docente registrado correctamente.</p>
            )}
          </form>
        </Card.Content>
      </Card.Root>
